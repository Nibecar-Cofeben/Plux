...
Respondé en español rioplatense, de forma concisa. Cuando el usuario pide hacer algo, HACELO con los comandos correspondientes además de explicar brevemente. Sos un agente que actúa, no solo habla. PODES USAR MÚLTIPLES ACCIONES DE UNA VEZ.`;
    ...
    // ================== AI CORE (FALLBACK CHAIN) ==================
    // ============================================================
    // AI CORE  Multi-provider fallback chain
    // 1. OpenRouter (FREE models)    No cost, many models
    // 2. Gemini 1.5 Flash            Fast & reliable
    // 3. Groq Llama3 70B             Excellent JSON compliance
    ...
    async function callAI(prompt, useJson = true) {
      ...
      try {
        ...
        console.warn(' AI_CORE: Error en Mistral:', e.message);
      }

      console.error(' AI_CORE: Todos los modelos fallaron.');
    ...
    function exportarPDF() {
      ...
      doc.text("Plux- Itinerario de Viaje", 14, 25);
      ...
      doc.text(`Nombre del viaje: ${document.getElementById('nombreViaje').value || 'Mi aventura'}`, 14, 45);
      doc.text(`Personas: ${numPersonas}`, 14, 52);
      doc.text(`Fecha inicio: ${document.getElementById('fechaInicio').value || 'No definida'}`, 14, 59);
      doc.text(`Origen: ${lugarSalida || 'No especificado'}`, 14, 66);
      
      let y = 75;
      destinos.forEach((dest, dIda) => {
        if (y > 250) { doc.addPage(); y = 20; }
        
        doc.setFontSize(16);
        doc.setTextColor(59, 130, 246);
        doc.text(`Destino ${dIda+1}: ${dest.nombre}`, 14, y);
        y += 10;
        doc.setTextColor(0,0,0);
        doc.setFontSize(10);
        let tramosBody = [];
        dest.tramos.forEach(t => {
          tramosBody.push([t.origen||'', t.destino||'', t.medio||'', t.precio?`${(t.precio*numPersonas).toFixed(2)}`:'']);
          if (t.escalas && t.escalas.length) {
            tramosBody.push([{ content: 'Escalas: '+t.escalas.join(', '), colSpan:4, styles:{fontStyle:'italic'} }]);
          }
        });
        if (tramosBody.length) {
          doc.autoTable({ startY: y, head: [['Origen','Destino','Medio','Costo']], body: tramosBody, theme:'striped', headStyles:{fillColor:[236,72,153]} });
          y = doc.lastAutoTable.finalY + 5;
        }
        dest.dias.forEach(dia => {
          if (!dia.eventos.length && !dia.costosAdicionales?.length) return;
          doc.setFontSize(12);
          doc.setTextColor(59,130,246);
          doc.text(`Día ${dia.id+1}`, 14, y);
          y += 6;
          doc.setTextColor(0,0,0);
          let eventosBody = [];
          dia.eventos.forEach(ev => {
            eventosBody.push([ev.hora||'--:--', ev.titulo||'Evento', ev.duracion?ev.duracion+' min':'', ev.costo?`${(ev.costo*numPersonas).toFixed(2)}`:'' ]);
          });
          if (dia.costosAdicionales) {
            dia.costosAdicionales.forEach(c => {
              if (c.descripcion) eventosBody.push(['--:--', c.descripcion, '', `${c.precio.toFixed(2)}`]);
            });
          }
          if (eventosBody.length) {
            doc.autoTable({ startY: y, head: [['Hora','Evento','Duración','Costo']], body: eventosBody, theme:'striped', headStyles:{fillColor:[59,130,246]} });
            y = doc.lastAutoTable.finalY + 5;
          } else {
            doc.text("Sin eventos", 14, y);
            y += 8;
          }
        });
        y += 5;
      ...
      if (vueltaGlobal || vueltaCostosAdicionales.length) {
        doc.setFontSize(14);
        doc.setTextColor(236,72,153);
        doc.text("Vuelta", 14, y);
        y += 8;
        doc.setFontSize(10);
        doc.setTextColor(0,0,0);
        let vueltaBody = [];
        if (vueltaGlobal) vueltaBody.push([vueltaGlobal, `${(vueltaPrecioGlobal*numPersonas).toFixed(2)}`]);
        vueltaCostosAdicionales.forEach(c => {
          if (c.descripcion) vueltaBody.push([c.descripcion, `${c.precio.toFixed(2)}`]);
        });
        if (vueltaBody.length) {
          doc.autoTable({ startY: y, head: [['Descripción','Costo']], body: vueltaBody, theme:'striped', headStyles:{fillColor:[236,72,153]} });
        }
      }
      doc.save('viaje-Plux.pdf');
    }
    ...
    function abrirPreferencias() {
        cerrarPanelHerramientas();
        let html = `<div id="modal-preferencias" style="position:fixed;inset:0;background:rgba(15,23,42,0.96);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;z-index:400;padding:20px;">
            <div style="background:var(--card);border:1px solid var(--border);border-radius:24px;padding:30px;max-width:500px;width:100%;">
                <h2 style="color:var(--verde);margin-bottom:20px;"> Preferencias Preferencias</h2>
                <div style="margin-bottom:20px;">
                    <label style="display:block;margin-bottom:8px;color:var(--gris);">Eventos por día (IA):</label>
                    <input type="number" id="pref-events" value="${userPreferences.eventsPerDay}" min="1" max="10" style="width:100%;padding:10px;background:var(--fondo);border:1px solid var(--border);border-radius:8px;color:white;">
                </div>
                <div style="margin-bottom:20px;">
                    <label style="display:block;margin-bottom:8px;color:var(--gris);">Ritmo del viaje:</label>
                    <select id="pref-pace" style="width:100%;padding:10px;background:var(--fondo);border:1px solid var(--border);border-radius:8px;color:white;">
                ...
                <div style="display:flex;gap:10px;margin-top:30px;">
                    <button onclick="guardarPreferencias()" style="flex:1;padding:15px;background:var(--verde);color:black;border:none;border-radius:12px;font-weight:bold;cursor:pointer;">Guardar</button>
                    <button onclick="document.getElementById('modal-preferencias').remove()" style="flex:1;padding:15px;background:var(--gris);color:white;border:none;border-radius:12px;cursor:pointer;">Cancelar</button>
                </div>
            </div>
        </div>`;
    ...
    function guardarPreferencias() {
        userPreferences.eventsPerDay = parseInt(document.getElementById('pref-events').value) || 4;
        userPreferences.pace = document.getElementById('pref-pace').value;
        autoSave();
